...
import styles from "./LightRaysBackground.module.scss";
...
const getAnchorAndDir = (
  ...
  // Keep the source offset stable across landscape and portrait viewports.
  const outside = Math.min(w, h) * 0.2;
  switch (origin) {
    case "top-left":
...
const LightRaysBackground: React.FC<LightRaysBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const frag = `precision highp float;
        ...
        float rayStrength(vec2 raySource, vec2 rayRefDirection, vec2 coord,
          ...
          float viewportScale = length(iResolution);
          float maxDistance = max(viewportScale * rayLength, 0.001);
          float lengthFalloff = clamp((maxDistance - distance) / maxDistance, 0.0, 1.0);

          float fadeRange = max(viewportScale * fadeDistance, 0.001);
          float fadeFalloff = fadeDistance > 0.0
            ? exp(-distance / fadeRange)
            : 1.0;
        ...
        void mainImage(out vec4 fragColor, in vec2 fragCoord) {
          ...
          fragColor.r *= mix(0.35, 0.9, brightness);
          fragColor.g *= mix(0.5, 0.9, brightness);
          fragColor.b *= mix(0.7, 1.0, brightness);
    ...
    const uniforms: Uniforms = {
      ...
      mouseInfluence: {
        value: config.followMouse ? config.mouseInfluence : 0.0,
      },
    ...
    const loop = (t: number) => {
      ...
      if (uniforms.mouseInfluence.value > 0.0) {
  ...
  useEffect(() => {
    ...
    u.mouseInfluence.value = followMouse ? mouseInfluence : 0.0;
  ...
  useEffect(() => {
    const handlePointerMove = (e: PointerEvent) => {
      if (!e.isPrimary || !containerRef.current || !rendererRef.current) return;
